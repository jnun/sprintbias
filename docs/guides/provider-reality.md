...
- 2026-09-03 — **A Claude exec crash was laundered into a "badly defined task" and parked in `blocked/`** (real-project failure, one task worked via a SprintBias install). Chain: the Claude CLI exited **0** while its result JSON reported `is_error:true, subtype:error_during_execution` (the mid-response-drop signature). `cli/claude.sh` detected `failed=1` but its retry gate was an **allowlist** — `error_during_execution` matched no transient pattern, so it neither retried nor surfaced; the terminal `return "$rc"` handed back **0**. `work.sh:_route_result` saw rc=0 + no `## Completed` and took the only remaining branch — stamped `incomplete`, moved to `blocked/` — which reads as "a human owes a decision" and caused a misdiagnosis. A **second, independent** hazard killed the rest of that queue: a resumed session finished its own move to `review/`, then `_route_result` (assuming `doing/`) called `move_file` on the vanished path and aborted the whole run under `set -e` (the call site was unguarded, unlike `_run_task`). Fixes: (1) claude.sh never returns 0 when `failed=1` — a 0-exit `is_error` is promoted to non-zero so it routes as the failure it is; (2) retry is now a **denylist** — resume unless fatal (KK/auth) or a deterministic cap (`_SPRINTBIAS_NONRETRY_RE`: turn/budget/flag), so a novel crash string retries once instead of dying silently; (3) `_route_result` is location-aware and crash-safe — it resolves where the file actually is, trusts a terminal folder the run already reached, never moves onto a missing path, and always returns 0; (4) a crash stays in `doing/` (the resumable state) stamped `failed` with a resume command instead of `blocked/` — **scope: a direct `./sprint.sh work` run only.** Under `loop`, `loop.sh:271-277` still sweeps all `doing/` residue → `blocked/` after each iteration, so a crash under `loop` is re-laundered there; reconciling loop's two contradictory `doing/` handlers (startup re-queues → `next/`; per-iteration → `blocked/`) is tracked in **#382**, not fixed here (deliberate: a half-fix to loop mid-incident would deepen that inconsistency); (5) a run-level JSONL journal (`docs/tmp/run-work-*.jsonl`) records the plan and every task transition. Claude-profile-specific: Grok omits the resume loop and has no `is_error` key, so nothing to port there. Disposition: fixed (docs/); unit-tested (classification + `_route_result` crash-safety under `set -e` + journal JSON validity). Follow-up open: `work --resume` consumer for the journal (data now exists; command not built).
